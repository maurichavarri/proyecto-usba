...
const AdminFixture = () => {
  ...
  const todosJugados =
    fixture.length > 0 &&
    fixture.every((partido) => partido.estado === "jugado");
  ...
  const obtenerFixture = async () => {
    try {
      const response = await fetch(
        `http://localhost:3000/api/v1/torneo-categorias/${id}/fixture`,
      );
  ...
  const obtenerTabla = async () => {
    try {
      const response = await fetch(
        `http://localhost:3000/api/v1/torneo-categorias/${id}/tabla`,
      );
      const data = await response.json();
      setTabla(data);
    } catch (error) {
  ...
  const obtenerResumen = async () => {
    try {
      const response = await fetch(
        `http://localhost:3000/api/v1/torneo-categorias/${id}/resumen`,
      );
      const data = await response.json();
      setResumen(data);
    } catch (error) {
  ...
  const obtenerDetalle = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `http://localhost:3000/api/v1/admin/torneo-categorias/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.message || "Error al obtener la competencia");
      }
      setDetalle(data);
    } catch (error) {
      console.error(error);
  ...
  const generarPlayoffs = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `http://localhost:3000/api/v1/torneo-categorias/${id}/playoffs`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const finalizarCompetencia = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `http://localhost:3000/api/v1/torneo-categorias/${id}/finalizar`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const partidosRegulares = fixture.filter(
    (partido) => !partido.fase || partido.fase === "regular",
  );
  const faseRegularFinalizada =
    partidosRegulares.length > 0 &&
    partidosRegulares.every((partido) => partido.estado === "jugado");
  const existeFinal = fixture.some((partido) => partido.fase === "final");
  const finalJugada = fixture.some(
    (partido) => partido.fase === "final" && partido.estado === "jugado",
  );
  const puedeFinalizarCompetencia =
    detalle?.formato_competencia === "solo_liga"
      ? todosJugados
      : existeFinal && finalJugada;
  ...
  return (
    <div className="container mt-5 mb-5">
      <div className="col-lg-10 mx-auto">
              ...
              <div className="col-md-6">
                <div className="card border-success">
                  <div className="card-body text-center">
                    ...
                    <h3 className="text-success">{detalle.campeon.nombre}</h3>
                  </div>
                </div>
              </div>
            ...
            <div className="col-md-2">
              <div className="card shadow-sm">
                <div className="card-body text-center">
                  ...
                  <h5 className="text-success">{resumen.partidosJugados}</h5>
                </div>
              </div>
            </div>
          </div>
                  ...
                  <tbody>
                    {tabla.map((equipo, index) => (
                      <tr key={equipo.equipo_id}>
                        ...
                        <td>
                          <span className="badge bg-success">{equipo.pts}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        {/* Sin fixture */}
        {fixture.length === 0 && (
          <div className="alert alert-secondary">No hay fixture generado.</div>
        )}
        ...
        {Object.keys(fasesPlayoff).length > 0 && (
          <>
            <hr className="my-5" />
            <h4 className="mb-3">Play-offs</h4>

            <div className="accordion" id="accordionPlayoffs">
              {Object.entries(fasesPlayoff).map(([fase, partidos], index) => {
                const tituloFase =
                  fase === "cuartos"
                    ? "Cuartos de Final"
                    : fase === "semifinal"
                ...
                return (
                  <div key={fase} className="accordion-item">
                    <h2 className="accordion-header">
                      <button
                        className={`accordion-button ${index !== 0 ? "collapsed" : ""}`}
                        type="button"
        ...
        {showHelp && (
          ...
          >
            ...
            >
              ...
              <p>
                Desde aquí podés visualizar y administrar el fixture generado
                para esta categoría del torneo.
              </p>
              ...
              <p className="mb-0">
                Una vez generados los partidos, las posiciones se actualizarán
                automáticamente a medida que se carguen los resultados.
              </p>
            </div>
          </div>
        )}
            ...
            >
              ...
              <div className="d-flex justify-content-center gap-2">
                <button
                  ...
                  onClick={() => setMostrarModalFixture(false)}
                >
                  Cancelar
                </button>
          ...
          >
            ...
            >
              <div className="text-center">
                ...
                <h4>Generar Play-Off</h4>

                <p>¿Estás seguro de que deseas generar la fase de Play-Off?</p>
              </div>

              <div className="alert alert-warning">
                Una vez generado el Play-Off, los equipos clasificados quedarán
                definidos según la tabla de posiciones de la fase regular.
              </div>
              ...
              <div className="d-flex justify-content-center gap-2">
                <button
                  type="button"
                  className="btn btn-secondary"
                  onClick={() => setMostrarModalPlayoff(false)}
                >
                  Cancelar
                </button>
          ...
          >
            ...
            >
              <div className="text-center">
                ...
                <h4>Finalizar Competencia</h4>

                <p>¿Estás seguro de que deseas finalizar la competencia?</p>
              </div>

              <div className="alert alert-danger">
                Esta acción marcará la competencia como finalizada. El campeón y
                subcampeón quedarán definidos y ya no se podrán realizar cambios
                deportivos sobre la competencia.
              </div>
